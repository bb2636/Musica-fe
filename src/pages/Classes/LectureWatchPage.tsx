...
const LectureWatchPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!urlParams.lectureId) {
            (async () => {
                try {
                    const list = await lectureApi.getLectureListForStudent(Number(urlParams.classId));
                    const first = list.find(l => l.isAccessible);
                    if (first) {
                        navigate(`/classes/${urlParams.classId}/lectures/${first.lectureId}`, { replace: true });
                    } else {
    ...
    const loadAllData = async () => {
        ...
        try {
            ...
            if (detail.watchedSeconds > 0) {
                setTimeout(() => {
                    if (videoRef.current) videoRef.current.currentTime = detail.watchedSeconds;
                }, 500);
            }
        } catch (err) {
    ...
    const getPreviousLecture = () => {
        const index = lectureList.findIndex(l => l.lectureId === lectureDetail?.lectureId);
    ...
    const getNextLecture = () => {
        const index = lectureList.findIndex(l => l.lectureId === lectureDetail?.lectureId);
        if (index >= 0 && index < lectureList.length - 1) return lectureList[index + 1];
        return null;
    };
    ...
    if (error || !lectureDetail) {
        return <div className="text-red-600">{error || '강의 정보를 불러올 수 없습니다.'}</div>;
    }
