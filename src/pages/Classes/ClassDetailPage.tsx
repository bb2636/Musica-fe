...
const ClassDetailPage = () => {
    ...
    const handleWishlistToggle = async () => {
        try {
            ...
            if (err instanceof Error) {
                ...
                errorMessage = axiosError.response?.data?.message || '로그인이 필요한 서비스입니다.';
            }
        ...
        try {
            ...
            if (typeof err === 'object' && err !== null && 'response' in err) {
                ...
                if (axiosError.response?.status === 401) {
                    ...
                    errorMessage = axiosError.response.data?.message || '이미 결제한 강의는 장바구니에 담을 수 없습니다.';
    ...
    const handlePurchase = () => {
        ...
        // 즉시 구매 처리
        navigate(`/payment?type=direct&classId=${classId}&price=${classDetail.classPrice}`);
    };
    ...
    const handleReviewUpdate = async (reviewId: number, data: { rating: number, comment: string }) => {
        try {
            ...
            <>
                ...
                <div className="flex flex-col items-center justify-center min-h-screen px-4">
                    <div className="text-center max-w-md">
                        ...
                        <p className="text-gray-600 mb-6">{error || '클래스를 찾을 수 없습니다.'}</p>
    ...
    const totalDuration = classDetail.lectures.reduce((sum, lecture) => sum + lecture.duration, 0);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-gray-600">
                            <span className="flex items-center gap-1">👨‍🏫 {classDetail.instructorName}</span>
                            <span className="flex items-center gap-1">📚 {classDetail.lectures.length}개 강의</span>
                            <span className="flex items-center gap-1">⏱️ 총 {formatDuration(totalDuration)}</span>
                        ...
                        {isEnrolled && classDetail.userClassStatus && (
                            <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                                <div className="flex justify-between text-sm text-green-800 mb-2">
                                    ...
                                    <span className="font-bold">{classDetail.userClassStatus.progressRate.toFixed(1)}%</span>
                                </div>
                                <div className="w-full bg-green-200 rounded-full h-3">
                                    <div
                                        className="bg-green-500 h-3 rounded-full transition-all duration-500"
                                        style={{ width: `${classDetail.userClassStatus.progressRate}%` }}
                                    ></div>
                                </div>
                                <div className="text-xs text-green-600 mt-2">
                                    {classDetail.userClassStatus.completedLectureCount} / {classDetail.userClassStatus.totalLectureCount} 강의 완료
                                </div>
                            </div>
                        )}
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                    ...
                    <div className={`text-gray-700 leading-relaxed ${showFullDescription ? '' : 'line-clamp-4'}`}>
                        <div dangerouslySetInnerHTML={{
                            __html: classDetail.descriptionHtml || '<p class="text-gray-500 italic">클래스 설명이 제공되지 않았습니다.</p>'
                        }} />
                    </div>
                    {classDetail.descriptionHtml && classDetail.descriptionHtml.length > 200 && (
                        <button
                    ...
                    <div className="bg-white rounded-lg shadow-sm border p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {reviewSummaries.map((summary, index) => (
                                <div key={index} className="bg-gradient-to-br from-blue-50 to-purple-50 p-4 rounded-lg border border-gray-100">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="flex">{renderStars(summary.rating)}</div>
                                        <span className="text-sm text-gray-600 font-medium">{summary.maskedUsername}</span>
                                    </div>
                                    <p className="text-sm text-gray-700 italic">"{summary.summary}"</p>
                                    <p className="text-xs text-gray-500 mt-2">{summary.createdAt}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        {reviews.length > 0 ? (
                            ...
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-lg font-semibold text-gray-600 mb-2">아직 등록된 후기가 없습니다</h3>
                                <p className="text-gray-500 mb-4">이 강의의 첫 번째 수강생이 되어보세요!</p>
                                {isEnrolled && (
                                    <button
                ...
                {!isEnrolled && (
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg p-8 text-center">
                        ...
                        <p className="text-blue-100 mb-6 text-lg">
                            {classDetail.lectures.length}개의 강의로 {classDetail.difficulty} 레벨의 {classDetail.categoryName}를 마스터하세요
                        </p>
