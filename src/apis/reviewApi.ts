...
export const reviewApi = {
  ...
  async getClassReviews(classId: number): Promise<ReviewDetail[]> {
    const response = await axiosInstance.get(`/reviews/classes/${classId}`);
    return response.data;
  },

  // 후기 요약 카드 (메인페이지용)
  async getReviewSummaryCards(): Promise<ReviewSummaryCard[]> {
    const response = await axiosInstance.get('/reviews/summary/cards');
    return response.data;
  },

  // 강의별 후기 요약 (AI)
  async getLectureSummary(lectureId: number): Promise<{ lectureId: string, summary: string }> {
    const response = await axiosInstance.get(`/reviews/summary/lecture/${lectureId}`);
    return response.data;
  },
  ...
  // 후기 수정 (인증 필요)
  async updateReview(reviewId: number, data: { rating: number, comment: string }): Promise<ReviewResponse> {
    const response = await axiosInstance.patch(`/users/reviews/${reviewId}`, data);
    return response.data;
  },
